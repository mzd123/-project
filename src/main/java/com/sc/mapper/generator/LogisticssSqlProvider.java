...
public class LogisticssSqlProvider {
    ...
    public String countByExample(LogisticssExample example) {
        SQL sql = new SQL();
        sql.SELECT("count(*)").FROM("TB_LOGISTICSS");
        applyWhere(sql, example, false);
        return sql.toString();
    }

    public String deleteByExample(LogisticssExample example) {
        SQL sql = new SQL();
        sql.DELETE_FROM("TB_LOGISTICSS");
        applyWhere(sql, example, false);
        return sql.toString();
    }

    public String insertSelective(Logisticss record) {
        SQL sql = new SQL();
    ...
    public String selectByExample(LogisticssExample example) {
        SQL sql = new SQL();
        if (example != null && example.isDistinct()) {
            sql.SELECT_DISTINCT("CM_LOGISTICSID");
        } else {
            sql.SELECT("CM_LOGISTICSID");
        }
        sql.SELECT("CM_LOGISTICSNAME");
        sql.FROM("TB_LOGISTICSS");
        applyWhere(sql, example, false);
    ...
    public String updateByExampleSelective(Map<String, Object> parameter) {
        Logisticss record = (Logisticss) parameter.get("record");
    ...
    protected void applyWhere(SQL sql, LogisticssExample example, boolean includeExamplePhrase) {
        if (example == null) {
            return;
        }
