...
public class AdminRoleSqlProvider {
    ...
    public String countByExample(AdminRoleExample example) {
        SQL sql = new SQL();
        sql.SELECT("count(*)").FROM("TB_ADMIN_ROLE");
    ...
    protected void applyWhere(SQL sql, AdminRoleExample example, boolean includeExamplePhrase) {
        if (example == null) {
            return;
        }
